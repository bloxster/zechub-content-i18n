## [ZODL](https://zodl.com)
![logo](https://github.com/user-attachments/assets/198608b2-9059-4cb7-aeb8-9354971376fd "ZODL")
- Devices: Mobile
- Operating System: Android | iOS
- Wallet Support: Frase seed | Viewing Key | Unified Address | Hardware
- Pools: Transparent | Sapling | Orchard
- Features: Rubrica indirizzi | CrossPay | DEX Swaps | Flexa Payments | MultiSignature | Richiesta di pagamento | Memo Shielded | Spend before Sync | TEX Address | Tor Support | F-droid

---

## [Ywallet](https://ywallet.app/installation/)
![logo](https://i.ibb.co/z4QxCWp/ywalletcard.png "Ywallet")
- Devices: Mobile | Desktop
- Operating System: Android | iOS | Windows | Linux | macOS
- Wallet Support: Frase seed | Viewing Key | Unified Address | Hardware
- Pools: Transparent | Sapling | Orchard
- Features: Rubrica indirizzi | Cold Storage | Diversified Address | Richiesta di pagamento | Pool Transfer | Memo Shielded | TEX Address | Voting | WarpSync

---

## [Dizzy](https://youtu.be/IVkuo1CGPBU)
![logo](https://i.ibb.co/FH918w9/Dizzy-3.png "Dizzy")
- Devices: Mobile | Desktop
- Operating System: Android | iOS | Windows | Linux | macOS
- Wallet Support: Frase seed | Viewing Key | Unified Address
- Pools: Sapling | Orchard
- Features: Discord Bot

---

## [Zingo!](https://www.zingolabs.org/)
![logo](https://i.ibb.co/bdJ49Ld/zingocard.png "Zingo!")
- Devices: Mobile | Desktop
- Operating System: Android | iOS | Windows | Linux
- Wallet Support: Frase seed | Viewing Key | Unified Address
- Pools: Transparent | Sapling | Orchard
- Features: Rubrica indirizzi | Financial Insights | Richiesta di pagamento | PepperSync | Memo Shielded | Supporto testnet

---

## [Edge](https://edge.app)
![logo](https://i.ibb.co/4FVQLS9/image-2024-11-28-091312254.png "Edge")
- Devices: Mobile
- Operating System: Android | iOS
- Wallet Support: Frase seed | Unified Address
- Pools: Transparent | Sapling
- Features: Shielding automatico | DEX Swaps | Multi-asset | Memo Shielded | Spend before Sync

---

## [Unstoppable](https://unstoppable.money)
![logo](https://i.ibb.co/C68mFCx/image-2024-11-28-091200540.png "Unstoppable")
- Devices: Mobile
- Operating System: Android | iOS
- Wallet Support: Frase seed | Unified Address
- Pools: Transparent | Sapling | Orchard
- Features: DEX Swaps | Multi-asset | Spend before Sync

---

## [eZcash](https://blog.nerdbank.net/ezcash-app)
![logo](https://i.ibb.co/C0q3jvw/e-Zcash-1.png "eZcash")
- Devices: Mobile | Desktop
- Operating System: Android | iOS | Windows
- Wallet Support: Frase seed | Viewing Key | Unified Address
- Pools: Transparent | Sapling | Orchard
- Features: Rubrica indirizzi | Address Check | Shielding automatico | Diversified Address | Richiesta di pagamento | Memo Shielded | TEX Address | Supporto testnet

---

## [Zkool](https://github.com/hhanh00/zkool2/)
![logo](https://i.ibb.co/BHGPff0K/zkool-1.png "Zkool")
- Devices: Mobile | Desktop
- Operating System: Android | iOS | Windows | Linux
- Wallet Support: Frase seed | Viewing Key | Unified Address | Hardware
- Pools: Transparent | Sapling | Orchard
- Features: Address Rotation | Broad Key Support | Encrypted Exports | FROST Multisig | Multi-Account Sync | Memo Shielded | Supporto testnet

---

## [Cake](https://cakewallet.com/install/)
![logo](https://github.com/user-attachments/assets/a8234c70-3714-4e80-b918-8739d45aea00 "Cake")
- Devices: Mobile | Desktop
- Operating System: Android | iOS | Windows | Linux | macOS
- Wallet Support: Frase seed | Viewing Key | Unified Address
- Pools: Transparent | Sapling | Orchard
- Features: Rubrica indirizzi | Address Rotation | Shielding automatico | DEX Swaps | Multi-asset | Memo Shielded | Tor Support

---

## [Zenith](https://code.vergara.tech/Vergara_Tech/zenith)
![logo](https://i.ibb.co/VtPBnm6/zenith-2.png "Zenith")
- Devices: Desktop
- Operating System: Windows | Linux | macOS
- Wallet Support: Frase seed | Viewing Key | Unified Address | Full Node
- Pools: Transparent | Sapling | Orchard
- Features: Rubrica indirizzi | Interfaccia grafica | Memo Shielded | Supporto testnet

---

## [Zingo-CLI](https://github.com/zingolabs/zingolib)
![logo](https://i.ibb.co/bdJ49Ld/zingocard.png "Zingo-CLI")
- Devices: Desktop
- Operating System: Windows | Linux | macOS
- Wallet Support: Frase seed | Viewing Key | Unified Address | Full Node
- Pools: Transparent | Sapling | Orchard
- Features: Interfaccia a riga di comando | PepperSync | Memo Shielded | Supporto testnet

---

## [Zallet](https://github.com/zcash/wallet)
![logo](https://i.ibb.co/3mCrR0dN/Brandmark-Yellow.png "Zallet")
- Devices: Desktop
- Operating System: Windows | Linux | macOS
- Wallet Support: Frase seed | Viewing Key | Unified Address | Full Node
- Pools: Transparent | Sapling | Orchard
- Features: Alpha Release | JSON-RPC Interface | Memo Shielded | Supporto testnet

---

## [Zcashd](https://github.com/zcash/zcash)
![logo](https://i.ibb.co/Xz2NskW/zcashd.png "Zcashd")
- Devices: Desktop
- Operating System: Windows | Linux | macOS
- Wallet Support: Frase seed | Viewing Key | Unified Address | Full Node
- Pools: Transparent | Sapling | Orchard
- Features: Interfaccia a riga di comando | Diversified Address | Memo Shielded | Supporto testnet

---

## [Brave](https://brave.com/web3-privacy/)
![logo](https://i.ibb.co/6yqMNwZ/image-2024-01-13-170934865.png "Brave")
- Devices: Web
- Operating System: Browser
- Wallet Support: Frase seed | Unified Address
- Pools: Transparent | Orchard
- Features: Memo Shielded | Spend before Sync | Supporto testnet

---

## [Metamask Snap](https://snaps.metamask.io/snap/npm/chainsafe/webzjs-zcash-snap/)
![logo](https://i.ibb.co/jn5t9Gy/metamask.png  "Metamask")
- Devices: Web
- Operating System: Browser
- Wallet Support: Frase seed | Unified Address
- Pools: Transparent | Sapling | Orchard
- Features: Multi-asset

---

## [Zucchini](https://zucchinifi.xyz/)
![logo](https://github.com/user-attachments/assets/91a1005a-3daf-4747-9442-c178cbe50e49 "Zucchini")
- Devices: Web
- Operating System: Browser
- Wallet Support: Frase seed | Unified Address
- Pools: Transparent | Sapling | Orchard
- Features: DEX Swaps

---

## [SSP Wallet](https://sspwallet.io)
![logo](https://i.ibb.co/X3hdfSY/ssp-1.png "SSP Wallet")
- Devices: Mobile | Web
- Operating System: Android | iOS | Browser
- Wallet Support: Frase seed
- Pools: Transparent
- Features: Multi-asset

---

## [Trust Wallet](https://trustwallet.com/download)
![logo](https://i.ibb.co/cNqxBJ1/trustwallet.png "Trust Wallet")
- Devices: Mobile | Web
- Operating System: Android | iOS | Browser
- Wallet Support: Frase seed
- Pools: Transparent
- Features: Multi-asset

---

## [Exodus](https://www.exodus.com/zcash-wallet-zec)
![logo](https://i.ibb.co/w0NNZNp/exoduscard.png "Exodus")
- Devices: Mobile | Desktop | Web
- Operating System: Android | iOS | Windows | Linux | macOS | Browser
- Wallet Support: Frase seed
- Pools: Transparent
- Features: Multi-asset

---

## [Coinomi](https://www.coinomi.com/en/downloads)
![logo](https://i.ibb.co/QfHmjWY/coinomicard.png "Coinomi")
- Devices: Mobile | Desktop
- Operating System: Android | iOS | Windows
- Wallet Support: Frase seed
- Pools: Transparent
- Features: Multi-asset

---

## [Keystone](https://shop.keyst.one)
![logo](https://i.ibb.co/c1gn635/Keystone-1.png "Keystone")
- Devices:
- Operating System:
- Wallet Support: Hardware | Unified Address
- Pools: Transparent | Sapling | Orchard
- Features: Multi-asset

---

## [Ledger](https://www.ledger.com/coin/wallet/zcash)
![logo](https://i.ibb.co/2qX6WCF/Desktop-Wallets.png "Ledger")
- Devices:
- Operating System:
- Wallet Support: Hardware
- Pools: Transparent
- Features: Multi-asset

---

## [Safepal](https://safepal.com)
![logo](https://i.ibb.co/h29h6d0/safepalcard.png "Safepal")
- Devices: Mobile | Web
- Operating System: Android | iOS | Browser
- Wallet Support: Frase seed | Hardware
- Pools: Transparent
- Features: Multi-asset

---

## [Trezor](https://trezor.io/learn/a/zcash-zec)
![logo](https://i.ibb.co/wcD52Kk/image-2024-...)
- Devices:
- Operating System:
- Wallet Support: Hardware
- Pools: Transparent
- Features: Multi-asset

## [Vultisig](https://vultisig.com)
![logo](https://i.ibb.co/93wSrxCc/vultisig.png "Vultisig")
- Devices: Web | Mobile | Desktop
- Operating System: Android | iOS | Windows | Linux | macOS | Browser
- Pools: Transparent
- Features: Multi-asset | App web

## [Vizor](https://vizor.cash/)
![logo](https://i.ibb.co/zhW2d2fV/Vizorphoto.png "Vizor")
- Devices: Desktop
- Operating System: macOS
- Wallet Support: Frase seed | Unified Address | Hardware
- Pools: Transparent | Sapling | Orchard
- Features: Shielding automatico | Memo Shielded | Supporto testnet | FROST Multisig

## [Noir wallet](https://www.zknoir.com/)
![logo](https://global.discourse-cdn.com/zcash/original/3X/3/b/3b88be907de788f367f7993bc24eed726d63a0c6.jpeg)
- Devices: Web | Desktop
- Operating System: Browser
- Pools: Shielded | Transparent
- Features: Zcash | Browser Extension | Private Beta | Shielded Transactions | Swap cross-chains | Prestiti e finanziamenti | DApp Connections | Native ZECFi

## [Leodex Wallet](https://leodex.io/)
![logo](https://leodex.io/assets/logo.webp)
- Devices: Web | Desktop
- Operating System: Browser | Windows | macOS
- Wallet Support: Frase seed | Wallet esterno | Indirizzo di ricezione | QR Code | Hardware
- Pools: Shielded | Transparent
- Features: Multi-asset | Swap cross-chain | Nessun KYC | Non custodial | App web | App desktop | Shielded ZEC | AnyPay
